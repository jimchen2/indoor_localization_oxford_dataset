...
def train_and_evaluate(args):
    ...
    log_dir = os.path.join("../../other_exp_logs", f"{current_time}_TCN_input{args.input_size}_channels{'-'.join(map(str, args.num_channels))}_output{args.output_size}_lr{args.learning_rate}_batch{args.batch_size}_dropout{args.dropout_rate}_sequencelength{args.sequence_length}")    
    ...
    model = IMUTCNModel(args.input_size, args.num_channels, args.kernel_size, args.dropout_rate, args.output_size).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_channels", type=int, nargs='+', default=[64, 32, 16], help="Number of channels in each layer")
